...
As listas a seguir contêm palavras-chave reservadas para uso atual ou futuro da
linguagem Rust. Portanto, elas não podem ser usadas como identificadores, exceto
como identificadores raw, como discutimos na seção [“Identificadores
...
A seguir está a lista das palavras-chave atualmente em uso, com a descrição da
funcionalidade de cada uma.

- **`as`**: Realiza cast primitivo, desambigua o trait específico que contém
  um item ou renomeia itens em instruções `use`.
...
- **`extern`**: Faz link com uma função ou variável externa.
...
- **`for`**: Itera sobre itens vindos de um iterador, implementa um trait ou
  especifica um lifetime de classificação superior.
...
- **`loop`**: Faz loop incondicionalmente.
- **`match`**: Faz match de um valor com padrões.
- **`mod`**: Define um módulo.
- **`move`**: Faz uma closure assumir ownership de todas as suas capturas.
...
- **`union`**: Define uma [union][union]<!-- ignore -->; é palavra-chave apenas
  quando usada em uma declaração de union.
...
nome da função quanto no lugar em que ela é chamada em `main`.
...
programas escritos em linguagens onde essas palavras não são palavras-chave.
Além disso, identificadores raw permitem usar bibliotecas escritas em uma
edição diferente do Rust daquela usada pelo seu crate. Por exemplo, `try` não é
palavra-chave na edição 2015, mas é nas edições 2018, 2021 e 2024. Se você

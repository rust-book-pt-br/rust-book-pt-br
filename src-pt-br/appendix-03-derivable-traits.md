...
seção [“Macros `derive`
personalizadas”][custom-derive-macros]<!-- ignore --> do Capítulo 20.
...
Um exemplo de quando `Eq` é necessário é no uso de chaves em um `HashMap<K,
V>`, para que o `HashMap<K, V>` possa determinar se duas chaves são iguais.

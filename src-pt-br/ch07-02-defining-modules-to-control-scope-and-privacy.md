...
- **Comece pela raiz do crate**: Ao compilar um crate, o compilador primeiro
  procura no arquivo raiz crate (geralmente _src/lib.rs_ para uma biblioteca crate e
  _src/main.rs_ para um crate binário) para código a ser compilado.
...
- **Declarando submódulos**: Em qualquer arquivo diferente da raiz crate, você pode
  ...
  - Inline, diretamente após ` mod vegetables`, entre colchetes
    do ponto e vírgula
...
- **Privado vs. público**: o código dentro de um módulo é privado de seu pai
...
- **A palavra-chave` use `**: dentro de um escopo, a palavra-chave` use `cria atalhos para
 ...
 ` crate::garden::vegetables::Asparagus `, você pode criar um atalho com` use
  ...
  escreva` Asparagus`para usar esse tipo no escopo.

Aqui, criamos um crate binário denominado `backyard` que ilustra essas regras.
O diretório do crate, também denominado _backyard_, contém esses arquivos e
diretórios:
...
O arquivo raiz crate neste caso é _src/main.rs_ e contém:
...
_Módulos_ nos permitem organizar o código em um crate para facilitar a leitura e reutilização.
...
Como exemplo, vamos escrever uma biblioteca crate que fornece a funcionalidade de um
...
restaurant --lib`. Em seguida, insira o código da Listagem 7-1 em _src/lib.rs_ para

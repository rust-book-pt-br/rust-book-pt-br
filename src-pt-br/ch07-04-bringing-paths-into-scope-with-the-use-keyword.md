...
módulo filho denominado`customer `, que é então um escopo diferente do` use`
...
não está claro onde ` add_to_waitlist`é definido.
...
Antes desta mudança, o código externo teria que chamar o `add_to_waitlist`
...
use `reexportou o módulo` hosting `do módulo raiz, o código externo
...
Número”][rand]<!-- ignore --> no Capítulo 2, trouxemos o ` Rng`trait para
...
use-o com ` use`para trazer itens de lá para o escopo do nosso pacote. Por exemplo,
...
Instruções `use`: uma que traz ` std::io`para o escopo e outra que traz
